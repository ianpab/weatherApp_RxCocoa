...
DelegateProxyType, CLLocationManagerDelegate {
    ...
    delegateProxy:
    RxCLLocationManagerDelegateProxy.self)
    }
    
    static func registerKnownImplementations() {
    register { RxCLLocationManagerDelegateProxy(locationManager:
    $0) }
    }
...
RxCLLocationManagerDelegateProxy.proxy(for: base)
    ...
    .filter { $0 == .authorizedWhenInUse || $0
    == .authorizedAlways } // 1
    .flatMap { _ in
    self.didUpdateLocations.compactMap(\.first) } // 2
    .take(1) // 3
